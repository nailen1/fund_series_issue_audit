...
def save_results_of_fund_series_issue(option_save=True):
    ...
    if option_save:
        # file_folder = FILE_FOLDER['result']
        # file_name = f'dataset-series_issue-at{get_yesterday().replace("-", "")}-save{get_today().replace("-", "")}.csv'
        # file_path = os.path.join(file_folder, file_name)
        # results.to_csv(file_path, encoding='utf-8')
        map_dataframe_to_csv_including_korean(results.reset_index(drop=True), file_folder=FILE_FOLDER['result'], file_name=f'dataset-series_issue-at{get_yesterday().replace("-", "")}-save{get_today().replace("-", "")}.csv')
    return results

def load_result_series_issue_audit(date_ref=None):
    regex = f'dataset-series_issue-at{date_ref.replace("-", "")}' if date_ref else 'dataset-series_issue-at'
    return open_df_in_file_folder_by_regex(file_folder=FILE_FOLDER['result'], regex=regex)
